...
!       V 4.1                           L. Ross Raszewski
!
! New in 4.1: Associative array lookup function
!             Glulx versions of pmove and rmove optimized to run faster
!             
...
#ifdef TARGET_GLULX;
[ objAddx o;
 return o+NUM_ATTR_BYTES+1;
];
#ifnot;
[ objAddx o;
  return (($0a)-->0)+126+((o-1)*14);
];
#endif;
[ SetParent x y;
 x=objAddx(x);
...
#ifdef TARGET_GLULX;
  remove obj1;
  if (~~child(obj2)) move obj1 to obj2;
  else
  {
  for(o=child(obj2):sibling(o):o=sibling(o));
  setsibling(o,obj1);
  setparent(obj1,obj2);
  }
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Assoc_Lookup - Lookup function for associative arrays
!  An associative array is a mapping from keys to values.
!  You declare an associative array like this:
! Array h --> NUMBER_OF_KEYS DEFAULT
!     KEY1 VALUE1
!     KEY2 VALUE2
!     ...
! Assoc_Lookup(KEY1,h) would return VALUE1. If the given key is not found,
! DEFAULT is returned
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
[ Assoc_Lookup v t i;
 for(i=2:i<2*((t-->0)+2):i=i+2)
  if (t-->i==v) return t-->(i+1);
 return t-->1;
];
