...
!
!	- Permite especificar �reas de b�squeda (para optimizar los c�lculos), para
...
!     pero entonces podr�a ser lento, dependiendo de las locs de la aventura.
! 	- Todas las localidades que se deseen tener en cuenta deben heredar de la 
!     clase 'Enrutable'
!	- Implementa el algoritmo de b�squeda Breath-First para el camino m�s corto 
!     (algoritmo �ptimo y completo)
!	- Las localidades que implementen una rutina como direcci�n, deber�n definir
!     en la propiedad ruta_dir_to una funci�n o valor que devuelva la localidad
!     de destino SIN imprimir ning�n mensaje.
!   - Debe definirse la constante MAX_LON_RUTA con el n�mero m�ximo de pasos
...
!      - NOMBRE_AREA : potencia de dos (o suma de) que identifica las �reas a 
!        buscar, o bien AREA_TODO para buscar en todo el mapa.
!      - array_destino(opc) : array donde se almacenan las direcciones, tendr�
!        tama�o MAX_LON_RUTA+1. O bien no especificar para que el resultado
!        se almacene en el array global "ruta_calculada"
!      - almacenar_locs(opc) : si se especifica un valor diferente de 0, se almacenar�n
...
! Se puede definir un �rea al que pertenezca una localidad, para optimizar las b�squedas dentro de
! una o varias �reas en vez de todo el mapa. Definir las �reas como constantes potencias de 2, ejemplo:
! Constant AREA_PUEBLO 1;
! Constant AREA_BOSQUE 2;
! Constant AREA_CUEVAS 4;
! ..etc..
! Por defecto las localidades se definen como AREA_TODO
! Se puede hacer la b�squeda sobre varias �reas sum�ndolas (AREA_BOSQUE+AREA_CUEVA)
...
with
	...
	ruta_prof 0,      ! N�mero de pasos desde el origen
	ruta_dir_from 0,       ! Direcci�n desde el padre en la ruta
...
[ calcula_ruta loc_origen loc_destino nombre_area array_destino almacenar_locs nivel tmp;
	#Ifdef DEBUG_RUTA;
	print "[RT: B�squeda de ruta entre ~", (name) loc_origen, "~ y ~", (name) loc_destino, "~]^";
	#EndIf;
	...
	switch (tmp)
	{
		0: ! No hay nodos => no se encuentra ruta
			...
			#EndIf;
			rfalse;
		2: ! Se encontr� el destino
			#Ifdef DEBUG_RUTA;
			print "[RT: Se encontr� el destino en el nodo inicial]^";
			#EndIf;
	...
	while (nivel < MAX_LON_RUTA)
	{
		nivel++;
		#Ifdef DEBUG_RUTA;
		print "[RT: Nivel de b�squeda:", nivel, "]^";
		#EndIf;
		...
		switch (tmp)
		{
			...
			2: ! Se encontr� el destino
				#Ifdef DEBUG_RUTA;
				print "[RT: Se encontr� el destino en el nivel actual]^";
				#EndIf;
				trazar_ruta(loc_origen, loc_destino, array_destino, almacenar_locs);
				rtrue;
		}
	...
	#Ifdef DEBUG_RUTA;
	print "[RT: Superada la longitud m�xima sin haber encontrado el destino]^";
	#EndIf;
...
!--------------------------------------------------------------------
! Un peque�o mapa para probar en modo DEBUG…. El laberinto de Zork
!---------------------------------------------------------------------
