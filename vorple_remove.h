...
! Functionalities taken from JustEnoughGlulx.h, by Roger Firth
! FIXME
...
#Endif; ! VORPLE_LIBRARY;
#Endif; ! LIBRARY_VERSION;



! To change the status line, add objects to 'StatusLineRulebook' with your
! code as the description of the object
...
! will get overwritten). To change the prompt, you can use the stub
! 'MyVorplePrompt()'' and return true when you're done.
! Do not use LibraryMessages with Prompt (the Vorple prompt won't be changed).
...
[ VorpleAppendToL__MPromptBefore;
	if (isVorpleSupported()) {
		bp_output_stream(3, Vorple_prompt, BUFLEN-1);
	}
];

[ VorpleAppendToL__MPromptAfter;
	if (isVorpleSupported()) {
		bp_output_stream(-3);
        ! if the prompt has changed since last turn...
        if (~~(compare_string(Vorple_prompt, BUFLEN-1, current_prompt))) {
...
[ VorpleAppendToL__MRestore n;
	! Fix for music not stopping after restoring a game
	if (n == 2 && isVorpleSupported()) {
		VorpleExecuteJavaScriptCommand(
			"$('.vorple-sound').remove();vorple.audio.stopMusic()"
		);
    }
...
! Hack to show notifications on the first turn.
! To display notifications on the first turn you must redefine the Inform
! 'LookRoutine' to add a call to 'VorpleAppendToLookRoutine()'
[ VorpleAppendToLookRoutine;
	! each_turn is executed at the end of every turn, so we need a way to
	! execute it at the very beginning too.
	! cf Roger Firth's "Why don't my daemons run at the start of the game"
	#Ifdef VORPLE_NOTIFICATIONS;
	if (turns == 0) {
		VorpleNotificationsFallback();
	}
	#Endif; ! VORPLE_NOTIFICATIONS;
];
...
!		vorple first, then the other extension.
!		For instance:
!
!			Include "vorple.h";
!			Include "parser";
...
!		these routines, conflicts will occur. You can declare the constant
!		"VORPLE_NO_REPLACES" to prevent Vorple from replacing these routines
!		-- you will have to manually add the Vorple modifications then.
!
!	Version:	0.0
!	Released:	2018/09/17
! ==============================================================================
...
[ do_vorple_handshake  exists str len       ix;
    ...
    exists = glk($0067, fref_handshake); ! fileref_does_file_exist
    if( ~~exists ) {
		rfalse; ! handshake file doesn't exist, not a Vorple interpreter
	}
    str = open_file(fref_handshake, HANDSHAKE_FILE, filemode_Write);
    write_stream(str, "Callooh!");
    glk($0044, str, gg_result); ! stream_close
...
[ IntToString i   j ;
	...
	! trim leading zeros
	i=0;
	while ( integer->(WORDSIZE+i) == '0' or 0) { i++; }
	for (j=i: j<(integer-->0): j++) {
		integer->(WORDSIZE+j-i) = integer->(WORDSIZE+j);
		integer->(WORDSIZE+j) = 0;
	}
...
! When we use print_to_array, '^' is replaced by a new line character, but
! Zcode uses 'carriage return' ('\r'), which was the norm on old micros (ZX,
! C64, etc) and Mac before OSX and Glulx uses 'line feed' ('\n'), like Unix.
! There are lots of norms and conventions out there: Windows uses '\r\n', which
! means Unix files sometimes appear in Windows as one big line. Anyway, it's
! just a big standardisation debate
#Ifdef TARGET_ZCODE;
Constant NEW_LINE_CHAR = 13;     ! '\r'
#Ifnot; ! TARGET_GLULX
Constant NEW_LINE_CHAR = 10;     ! '\n'
...
[ VorpleWhatType txt        len c d;
    ...
    if ( txt->(WORDSIZE) == 'f' && txt->(WORDSIZE) == 'u'
		&& txt->(WORDSIZE) == 'n' && txt->(WORDSIZE) == 'c'
		&& txt->(WORDSIZE) == 't' && txt->(WORDSIZE) == 'i'
		&& txt->(WORDSIZE) == 'o' && txt->(WORDSIZE) == 'n'
		&& txt->(WORDSIZE) == ' ') {
        return "function";
    }
    ! corresponding regexp : "^\-?\d+(\.\d+)?$"
...
[ VorpleConvertTextIntoNumber txt     s l neg prev res n c d;
    ...
    for (n=s: n<l: n++) {
        ...
        if (c == '.') {
			...
			!		if negated is false:
			!			increment result;
			!		otherwise unless T exactly matches the regular expression
			!			"\-\d+\.50*":
			!			increment result;
			!	break;
        }
